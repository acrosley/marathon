...
Phase 0 (systems groundwork) is done; Phase 1 (self-hosted vLLM, warm-tier KV reuse) has met its exit criterion. This repo currently provides the deterministic core: canonical byte-stable serialization with an append-only history guarantee, a hash-chained snapshot ledger with tamper detection, an rsync-style byte-matched delta engine, a turn protocol with cryptographic integrity verification, and a benchmark harness that measures delta savings against full resend. A session runner drives real conversations through the ledger, and a CI gate proves delta-reconstructed state is byte-identical to full-context replay at every turn. Phase 1 is written up end to end — method, results, limitations, reproduction — in [docs/phase1-report.md](docs/phase1-report.md). `marathon.server` / `marathon.client` run the whole protocol end to end over HTTP (12.6k-token edit turn 0.18 s vs 1.28 s; repeated edits at the same speed).

Phase 2 (cold tier + recall-on-miss) is built and measured: the active window stays bounded (≈8k of a 35k history) and recall-on-miss restores demoted facts (exact-match 0.817 vs 0.942 with full context; the right message is promoted 91% of the time) — a quality regression we call partial, not a pass. Paging through the shift connector is 2.6× faster per reused turn but not yet accurate at depth: every wrong turn is a reuse turn and the damage tracks how much of the reused span's context has since churned (coordinates are proven exact), so a token-churn ceiling is the open work. Phase 3 (the trust contract) is reframed as stitched-KV consistency fine-tuning; three LoRA iterations on Qwen3-8B show the failure tail is trainable and collateral can be held with a do-no-harm hinge, but at the 4–8k regime the tail does not yet close and the metric needs paired, bootstrapped estimates before more GPU is spent. Status detail: [docs/PLAN.md](docs/PLAN.md); every number: [docs/findings.md](docs/findings.md).
