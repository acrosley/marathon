...
Written before the GPU was touched, because iteration 2 ended with two arms that failed *differently* — arm B (hinge + `--grad-prefill`) bought the tail (ratio 1.56× mean, every item over KL 0.05 eliminated) and arm A (hinge, cheap prefill) protected the collateral (clean drift 0.0019, the smaller non-governing median regression). "Arm B's signal with arm A's restraint" is a hypothesis about a knob, and a knob turned after seeing the result is not a test. So the knob, the stopping rule, the regime and the new bucket are all fixed here first.

**Regime: 4–8k, not 4–6k.** Iteration 2's ratios are not comparable to iteration 1's because the base failure is milder where they were measured: base ratio 3.74× mean / 3.51× median at 4–6k against 7.25× / 2.27× (n=60) and 8.71× / 4.03× (n=48) at 4–8k. The ≤2× target was set against a ~9× base, so the gate is only honestly tested where the base is that bad. Every iteration-3 number is 4–8k, and the run opens with a base-only eval at n≥120 to record what the base ratio actually is on this population before anything is trained.

**Hinge weight: one ordered sweep, `w ∈ {1, 2, 4}`, `w=2` first and `w=4` only if the clock allows.** `w=1` is iteration 2 and is not re-run; it is the point already on the curve. The ordering is fixed rather than chosen after the first result: `w=2` is the smaller extrapolation from a hinge that already cut the regression from +37%/+46% to +13%/+25%, and `w=4` is the fallback if doubling it does not buy the non-governing median back. Both arms carry `--grad-prefill` (arm B's signal) and the do-no-harm hinge (arm A's restraint) — that combination *is* the iteration, and the hinge weight is the only free variable. **Hard stop on GPU work at 17:30 local**; if only one arm fits, it is `w=2`, and the entry reports one arm.

**Checkpoint-selection rule.** Adapters are saved every 50 items with a held-out mid-training eval, and the checkpoint that gets evaluated at n≥120 is chosen by this rule, applied to the mid-training slice:

> **Pick the checkpoint with the best governing p95, subject to non-governing median ≤ base × 1.2 on the same items and clean drift ≤ 0.002.**

The objective is the tail because the tail is what forces `reuse_plan` to refuse; the two constraints are criteria 3 and 2 restated at their own thresholds, so the rule cannot select a checkpoint that would fail them by construction. It is implemented as `stitch_train.select_checkpoint` with the thresholds as module constants, and the run prints its choice — not read off a curve by eye afterwards. **The rule is allowed to select nothing**, which is a reportable outcome (every measured point bought the tail by spending the collateral) and is not to be repaired by loosening the thresholds after the fact. If it selects nothing, the final checkpoint is evaluated and reported as such.

Caveat recorded in advance: the mid-training slice is 24 items, so its p95 is a 24-item statistic and the selection is noisy. It is still preferable to selecting by eye, and the n≥120 held-out eval that follows is the number the gates are read from.

**`--grad-prefill` at 8k.** Iteration 2 ran it at 4–6k with the cap at 8000 (so no item fell back) and peaked at 27.4 GiB. The dominant term is full-length K and V per layer, which is linear in context: at 8B/bf16 that is ~147 KiB per token, so ~1.2 GiB per full-length copy at 8k against ~0.9 GiB at 6k, over the three copies live at once (`old_kv`, the placed stitched cache, and the `index_copy` result SDPA saves for backward). Extrapolating iteration 2's peak gives roughly 28–29 GiB at 8k on a 32 GiB card: it should fit, with little headroom, so the cap is raised to cover an 8k session and **three** things make that safe rather than optimistic. `stitch_memory_estimate` prints the predicted full-length term and the number of items over the cap before the first step. The per-item `torch.cuda.max_memory_allocated` peak is logged and printed, so the run reports what it actually used. And an item that OOMs on the expressive path now **falls back to the cheap path for that item and is counted**, instead of ending the run — which is how the first 8B attempt died 81 items in, with WSL surfacing the exhaustion as `CUDA driver error: device not ready` rather than as a clean OOM. Gradient checkpointing is still not used: re-running a decoder layer during backward would re-enter this module's cache scatter, and the fallback is a bound that needs no such argument.

**A new training bucket: `standing-governing`.** The `dep-instruction` probe got *worse* in both iteration-2 arms, and the stated reason was a distribution gap rather than a failure of the method — the probe is a homogeneous log of `Log entry t.` turns with the standing instruction in an early user turn and open-ended questions whose *answer form* the instruction governs, none of which the training population contained. Criterion 4 gates on that probe, so training that never sees its shape is being asked for off-distribution generalisation and then failed for not delivering it. `kvshift_eval.build_standing_item` builds exactly that shape — the probe's own filler generator, the instruction in the system prompt on even `sid` and in an early governing user turn on odd `sid`, open-ended questions with no forced prefix, one single-span edit that flips the instruction — but sized by the real tokenizer to the same 4–8k regime as everything else. It enters **both** the training mix and the held-out eval, at `--standing-frac 0.34` of the governing half.

Two things about how it is reported, fixed now so the comparison stays honest. It is **its own bucket** in the table, and the headline gate ratio stays on the core population (`governing` + `mid-governing`) that iterations 1 and 2 measured, with a second `+std` ratio reported beside it — a bucket added mid-experiment must not silently move the number the gates are read from. And it is a **governing** edit by every definition the project uses (the flag, the mechanism, the failure), so it is not a way of quietly diluting the non-governing bucket either.

What this does *not* do is make criterion 4 easy: the probe scenarios themselves are still held out — different builder, different text, 20 fixed turns, `dep-anaphora` and `dep-contradict` untouched as controls — so the criterion still measures transfer, just from a training set that contains the shape rather than one that does not. If `dep-instruction` improves while the controls degrade, that is the overfitting risk named at the top of the risks section arriving on schedule, and it should be reported as such.
