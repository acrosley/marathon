...
**The next run, specified:** Qwen3-8B, 4–8k sessions, the full query pool including `obey`, and a **base-only eval first** to confirm the ~9× reproduces before spending an epoch on it. Fix `build_examples` to sample across query types. Lower the learning rate — `clean_kl` spiked to 0.0223 by step 81 at 1e-4, which is too hot for r=16 on all four projections.
