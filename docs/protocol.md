...
Sessions are isolated end to end: previous state, per-line token cache and connector store key are all keyed by session id. The connector key carries an **epoch** (`"<id>#<n>"`). The store is written while a session is append-only; an edit turn reads it and does not write it, and the epoch then rolls so later turns re-save under a fresh key instead of layering new positions over the pre-edit layout. Coverage the new epoch lacks is declined by the store, so a second edit in one session costs a recompute, never a wrong answer — v1 accelerates the first edit in a session and degrades safely after it.
